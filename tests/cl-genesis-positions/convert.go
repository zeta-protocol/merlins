...
func ConvertSubgraphToMerlinsGenesis(positionCreatorAddresses []sdk.AccAddress, subgraphFilePath string) (*clgenesis.GenesisState, *banktypes.GenesisState) {
	...
	pool, err := merlins.App.ConcentratedLiquidityKeeper.GetPoolFromPoolIdAndConvertToConcentrated(merlins.Ctx, poolId)
	if err != nil {
		panic(err)
	}
	...
	_, _, _, _, _, err = merlins.App.ConcentratedLiquidityKeeper.CreateFullRangePosition(merlins.Ctx, pool.GetId(), merlins.TestAccs[0], sdk.NewCoins(sdk.NewCoin(msgCreatePool.Denom0, sdk.NewInt(100)), sdk.NewCoin(msgCreatePool.Denom1, sdk.NewInt(100))))
	if err != nil {
		panic(err)
	}
	...
	for _, uniV3Position := range positions {
		...
		lowerTickMerlins, err := math.PriceToTickRoundDown(lowerPrice, pool.GetTickSpacing())
		if err != nil {
			panic(err)
		}

		upperTickMerlins, err := math.PriceToTickRoundDown(upperPrice, pool.GetTickSpacing())
		if err != nil {
			panic(err)
		}

		if lowerTickMerlins.GT(upperTickMerlins) {
			fmt.Printf("lowerTickMerlins (%s) > upperTickMerlins (%s), skipping", lowerTickMerlins, upperTickMerlins)
			continue
		}

		if lowerTickMerlins.Equal(upperTickMerlins) {
			// bump up the upper tick by one. We don't care about having exactly the same tick range
			// Just a roughly similar breakdown
			upperTickMerlins = upperTickMerlins.Add(sdk.OneInt())
		}
		...
		position, err := clMsgServer.CreatePosition(sdk.WrapSDKContext(merlins.Ctx), &cltypes.MsgCreatePosition{
			...
			LowerTick:       lowerTickMerlins.Int64(),
			UpperTick:       upperTickMerlins.Int64(),
		...
		bigBangPositions = append(bigBangPositions, BigBangPosition{
			...
			JoinTime:   merlins.Ctx.BlockTime().Format("2006-01-02T15:04:05Z"), // ISO 8601
			Liquidity:  position.LiquidityCreated.String(),
			PositionID: strconv.FormatUint(position.PositionId, 10),
			LowerTick:  lowerTickMerlins.String(),
			UpperTick:  upperTickMerlins.String(),
		})
	}
...
func parseStringToInt(strInt string) (result sdk.Int, failedParsing bool) {
	...
	result = furymath.MustNewDecFromStr(strInt).SDKDec().MulInt64(int64(merlinsPrecision)).TruncateInt()
...
func writeStateToDisk(state map[string]json.RawMessage) {
	...
	err = os.WriteFile(pathToFilesFromRoot+merlinsGenesisFileName, stateBz, 0644)
	if err != nil {
		panic(err)
	}
}
