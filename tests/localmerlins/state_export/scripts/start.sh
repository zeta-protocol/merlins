...
then
    ...
    echo $MNEMONIC | merlinsd init -o --chain-id=$CHAIN_ID --home $MERLINS_HOME --recover $MONIKER 2> /dev/null
    echo $MNEMONIC | merlinsd keys add my-key --recover --keyring-backend test > /dev/null 2>&1

    ACCOUNT_PUBKEY=$(merlinsd keys show --keyring-backend test my-key --pubkey | dasel -r json '.key' --plain)
    ACCOUNT_ADDRESS=$(merlinsd keys show -a --keyring-backend test my-key --bech acc)

    VALIDATOR_PUBKEY_JSON=$(merlinsd tendermint show-validator --home $MERLINS_HOME)
    VALIDATOR_PUBKEY=$(echo $VALIDATOR_PUBKEY_JSON | dasel -r json '.key' --plain)
    VALIDATOR_HEX_ADDRESS=$(merlinsd debug pubkey $VALIDATOR_PUBKEY_JSON 2>&1 --home $MERLINS_HOME | grep Address | cut -d " " -f 2)
    VALIDATOR_ACCOUNT_ADDRESS=$(merlinsd debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $MERLINS_HOME | grep Acc | cut -d " " -f 3)
    VALIDATOR_OPERATOR_ADDRESS=$(merlinsd debug addr $VALIDATOR_HEX_ADDRESS 2>&1  --home $MERLINS_HOME | grep Val | cut -d " " -f 3)
    VALIDATOR_CONSENSUS_ADDRESS=$(merlinsd debug bech32-convert $VALIDATOR_OPERATOR_ADDRESS -p furyvalcons  --home $MERLINS_HOME 2>&1)
...
merlinsd start --home $MERLINS_HOME --x-crisis-skip-assert-invariants
