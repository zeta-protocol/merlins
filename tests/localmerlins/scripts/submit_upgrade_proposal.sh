...
MERLINS_HOME=$HOME/.merlinsd-local/
...
make_proposal() {
    ...
    MERLINS_CMD="merlinsd tx gov submit-proposal software-upgrade \
        $UPGRADE_VERSION \
...
# Query proposal
query_proposal() {
    merlinsd q gov proposal $PROPOSAL_ID \
        --node $RPC_NODE \
        -o json | jq
}
...
vote_on_proposal() {
    ...
    MERLINS_CMD="merlinsd tx gov vote $PROPOSAL_ID yes \
        --from $KEY \
        --chain-id $CHAIN_ID \
        --fees $TX_FEES \
        --node $RPC_NODE \
