...
add_genesis_accounts () {
    ...
    merlinsd add-genesis-account fury12smx2wdlyttvyzvzg54y2vnqwq2qjateuf7thj 100000000000ufury,100000000000uion,100000000000stake,100000000000uusdc,100000000000uweth --home $MERLINS_HOME
    merlinsd add-genesis-account fury1cyyzpxplxdzkeea7kwsydadg87357qnahakaks 100000000000ufury,100000000000uion,100000000000stake,100000000000uusdc,100000000000uweth --home $MERLINS_HOME
    merlinsd add-genesis-account fury18s5lynnmx37hq4wlrw9gdn68sg2uxp5rgk26vv 100000000000ufury,100000000000uion,100000000000stake,100000000000uusdc,100000000000uweth --home $MERLINS_HOME
    merlinsd add-genesis-account fury1qwexv7c6sm95lwhzn9027vyu2ccneaqad4w8ka 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury14hcxlnwlqtq75ttaxf674vk6mafspg8xwgnn53 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury12rr534cer5c0vj53eq4y32lcwguyy7nndt0u2t 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury1nt33cjd5auzh36syym6azgc8tve0jlvklnq7jq 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury10qfrpash5g2vk3hppvu45x0g860czur8ff5yx0 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury1f4tvsdukfwh6s9swrc24gkuz23tp8pd3e9r5fa 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury1myv43sqgnj5sm4zl98ftl45af9cfzk7nhjxjqh 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury14gs9zqh8m49yy9kscjqu9h72exyf295afg6kgk 100000000000ufury,100000000000uion,100000000000stake --home $MERLINS_HOME
    merlinsd add-genesis-account fury1jllfytsz4dryxhz5tl7u73v29exsf80vz52ucc 1000000000000ufury,1000000000000uion,1000000000000stake --home $MERLINS_HOME

    echo $MNEMONIC | merlinsd keys add $MONIKER --recover --keyring-backend=test --home $MERLINS_HOME
    echo $POOLSMNEMONIC | merlinsd keys add pools --recover --keyring-backend=test --home $MERLINS_HOME
    merlinsd gentx $MONIKER 500000000ufury --keyring-backend=test --chain-id=$CHAIN_ID --home $MERLINS_HOME
...
create_two_asset_pool() {
  run_with_retries "merlinsd tx gamm create-pool --pool-file=$1 --from pools --chain-id=$CHAIN_ID --home $MERLINS_HOME --keyring-backend=test -b block --fees 5000ufury --yes" "create two asset pool: successful"
}

create_three_asset_pool() {
  run_with_retries "merlinsd tx gamm create-pool --pool-file=nativeDenomThreeAssetPool.json --from pools --chain-id=$CHAIN_ID --home $MERLINS_HOME --keyring-backend=test -b block --fees 5000ufury --gas 900000 --yes" "create three asset pool: successful"
}

if [[ ! -d $CONFIG_FOLDER ]]
then
    echo $MNEMONIC | merlinsd init -o --chain-id=$CHAIN_ID --home $MERLINS_HOME --recover $MONIKER
    install_prerequisites
    edit_genesis
    add_genesis_accounts
    edit_config
...
then
    ...
    go build \
        ...
        -ldflags \
            ...
            -X github.com/cosmos/cosmos-sdk/version.AppName="merlinsd" \
