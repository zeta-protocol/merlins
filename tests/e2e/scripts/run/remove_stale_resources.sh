...
# Filtering by containers belonging to the "merlins-testnet" network
LIST_CONTAINERS_CMD=$(docker ps -a --filter network=merlins-testnet --format {{.ID}})
LIST_NETWORKS_CMD=$(docker network ls --filter name=merlins-testnet --format {{.ID}})
...
for cur_image_sha in $LIST_DOCKER_IMAGE_HASHES; do
    if [[ "$cur_image_sha" != "$local_git_sha" ]]; then
        ...
        docker rmi -f $(docker images --filter=reference="merlins:$cur_image_sha" -q)
    fi
done
