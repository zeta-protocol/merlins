...
import (
	...
	"github.com/merlins-labs/merlins/v15/tests/e2e/configurer/chain"
	...
	ibchookskeeper "github.com/merlins-labs/merlins/x/ibc-hooks/keeper"

	ibcratelimittypes "github.com/merlins-labs/merlins/v15/x/ibc-rate-limit/types"
	poolmanagertypes "github.com/merlins-labs/merlins/v15/x/poolmanager/types"

	sdk "github.com/cosmos/cosmos-sdk/types"
	coretypes "github.com/tendermint/tendermint/rpc/core/types"

	"github.com/merlins-labs/merlins/furyutils/furyassert"
	appparams "github.com/merlins-labs/merlins/v15/app/params"
	v16 "github.com/merlins-labs/merlins/v15/app/upgrades/v16"
	...
	cl "github.com/merlins-labs/merlins/v15/x/concentrated-liquidity"
...
func (s *IntegrationTestSuite) TestConcentratedLiquidity() {
	...
	var (
		// Swap parameters
		ufuryInDec_Swap1 = sdk.NewDec(3465198)
		ufuryIn_Swap1    = fmt.Sprintf("%sufury", ufuryInDec_Swap1.String())
	)
	...
	feeGrowthGlobal.AddMut(calculateFeeGrowthGlobal(ufuryInDec_Swap1, swapFeeDec, concentratedPool.GetLiquidity()))
	...
	inAmountSubFee := ufuryInDec_Swap1.Mul(sdk.OneDec().Sub(swapFeeDec))
	...
	// Assert
	s.Require().Equal(
		addr1BalancesBefore.AmountOf("ufury").Add(feesUncollectedAddress1Position1_Swap1.TruncateInt()).String(),
	...
	var (
		...
		// ufuryInDec_Swap2_NoFee is calculated such that swapping this amount (not considering fee) moves the price over the next initialized tick
		ufuryInDec_Swap2_NoFee = amountInToGetToNextInitTick.Add(amountInToGetToTickAfterInitialized)
		ufuryInDec_Swap2       = ufuryInDec_Swap2_NoFee.Quo(sdk.OneDec().Sub(swapFeeDec)).TruncateDec() // account for swap fee of 1%
	...
	// Step1: amountIn is ufury tokens that are swapped + ufury tokens that are paid for fee
	...
	amountRemainingAfterStep1 := ufuryInDec_Swap2.Sub(amountInToGetToNextInitTick).Sub(feeCharge_Swap2_Step1)
	...
	// Assert
	s.Require().Equal(
		addr1BalancesBefore.AmountOf("ufury").Add(feesUncollectedAddress1Position1_Swap2.TruncateInt()),
	...
	// Assert
	s.Require().Equal(
		addr3BalancesBefore.AmountOf("ufury").Add(totalUncollectedFeesAddress3Position2.TruncateInt()),
...
func (s *IntegrationTestSuite) TestGeometricTwapMigration() {
	...
	node.BankSend(ufuryIn, chainA.NodeConfigs[0].PublicAddress, swapWalletAddr)

	// Swap to create new twap records on the pool that was created pre-upgrade.
	node.SwapExactAmountIn(ufuryIn, minAmountOut, fmt.Sprintf("%d", config.PreUpgradePoolId), otherDenom, swapWalletAddr)
}
...
func (s *IntegrationTestSuite) TestIBCTokenTransferRateLimiting() {
	...
	furySupply, err := node.QuerySupplyOf("ufury")
	s.Require().NoError(err)
	...
	// Sending >1%
	chainA.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, sdk.NewInt64Coin(initialization.FuryDenom, int64(over)))
	...
	chainA.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, sdk.NewInt64Coin(initialization.FuryDenom, 1))
	// Sending >1%. Should fail
	node.FailIBCTransfer(initialization.ValidatorWalletName, chainB.NodeConfigs[0].PublicAddress, fmt.Sprintf("%dufury", int(over)))

	// Removing the rate limit so it doesn't affect other tests
	node.WasmExecute(contract, `{"remove_path": {"channel_id": "channel-0", "denom": "ufury"}}`, initialization.ValidatorWalletName)
...
func (s *IntegrationTestSuite) TestIBCWasmHooks() {
	...
	nodeB.SendIBCTransfer(validatorAddr, contractAddr, fmt.Sprintf("%dufury", transferAmount),
...
func (s *IntegrationTestSuite) TestPacketForwarding() {
	...
	nodeA.SendIBCTransfer(validatorAddr, validatorAddr, fmt.Sprintf("%dufury", transferAmount), string(forwardMemo))

	// check the balance of the contract
	s.CheckBalance(nodeA, contractAddr, "ufury", transferAmount)
...
func (s *IntegrationTestSuite) TestArithmeticTWAP() {
	...
	furyassert.DecApproxEq(s.T(), twapFromBeforeSwapToBeforeSwapOneAB, twapFromBeforeSwapToBeforeSwapTwoAB, sdk.NewDecWithPrec(1, 3))
	furyassert.DecApproxEq(s.T(), twapFromBeforeSwapToBeforeSwapOneBC, twapFromBeforeSwapToBeforeSwapTwoBC, sdk.NewDecWithPrec(1, 3))
	furyassert.DecApproxEq(s.T(), twapFromBeforeSwapToBeforeSwapOneCA, twapFromBeforeSwapToBeforeSwapTwoCA, sdk.NewDecWithPrec(1, 3))
	...
	furyassert.DecApproxEq(s.T(), twapAfterSwapBeforePruning10MsAB, twapFromAfterToNowAB, sdk.NewDecWithPrec(2, 3))
	furyassert.DecApproxEq(s.T(), twapAfterSwapBeforePruning10MsBC, twapFromAfterToNowBC, sdk.NewDecWithPrec(2, 3))
	furyassert.DecApproxEq(s.T(), twapAfterSwapBeforePruning10MsCA, twapFromAfterToNowCA, sdk.NewDecWithPrec(2, 3))
	...
	s.Require().NoError(err)
	// There are potential rounding errors requiring us to approximate the comparison.
	furyassert.DecApproxEq(s.T(), twapToNowPostPruningAB, twapAfterSwapBeforePruning10MsAB, sdk.NewDecWithPrec(1, 3))
	furyassert.DecApproxEq(s.T(), twapToNowPostPruningBC, twapAfterSwapBeforePruning10MsBC, sdk.NewDecWithPrec(1, 3))
	furyassert.DecApproxEq(s.T(), twapToNowPostPruningCA, twapAfterSwapBeforePruning10MsCA, sdk.NewDecWithPrec(1, 3))
}
...
func (s *IntegrationTestSuite) TestGeometricTWAP() {
	...
	// ufury = 2_000_000
	// quote assset supply / base asset supply = 1_000_000 / 2_000_000 = 0.5
	furyassert.DecApproxEq(s.T(), sdk.NewDecWithPrec(5, 1), afterSwapTwapBOverA, sdk.NewDecWithPrec(1, 2))
}
...
func (s *IntegrationTestSuite) TestAConcentratedLiquidity_CanonicalPool_And_Parameters() {
	...
	var (
		// Taken from: https://app.merlins.zone/pool/674
