...
production merlins container to start the `merlinsd` service.
...
1. Executing commands on docker containers
    ...
    Example: `docker exec fury-test-a-node-prune-nothing-snapshot merlinsd status` will print a node status of `fury-test-a-node-prune-nothing-snapshot` container. 
