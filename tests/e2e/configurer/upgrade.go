...
import (
	...
	"github.com/merlins-labs/merlins/v15/tests/e2e/containers"
...
func (uc *UpgradeConfigurer) CreatePreUpgradeState() error {
	...
	// Create DAI/FURY pool from v16 migration testing with superfluid enabled.
	config.DaiFuryPoolIdv16 = chainANode.CreateBalancerPool("daifuryv16.json", initialization.ValidatorWalletName)
	daiFuryShareDenom := fmt.Sprintf("gamm/pool/%d", config.DaiFuryPoolIdv16)
	chainA.EnableSuperfluidAsset(daiFuryShareDenom)
