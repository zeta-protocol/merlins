...
build: check_version go.sum
	mkdir -p $(BUILDDIR)/
	GOWORK=off go build -mod=readonly  $(BUILD_FLAGS) -o $(BUILDDIR)/ $(GO_MODULE)/cmd/merlinsd
...
install: check_version go.sum
	GOWORK=off go install -mod=readonly $(BUILD_FLAGS) $(GO_MODULE)/cmd/merlinsd
...
build-reproducible-amd64: go.sum
	...
	$(DOCKER) cp furybinary:/bin/merlinsd $(BUILDDIR)/merlinsd-linux-amd64
...
build-reproducible-arm64: go.sum
	...
	$(DOCKER) cp furybinary:/bin/merlinsd $(BUILDDIR)/merlinsd-linux-arm64
	$(DOCKER) rm -f furybinary
...
draw-deps:
	...
	@goviz -i ./cmd/merlinsd -d 2 | dot -Tpng -o dependency-graph.png
...
PACKAGES_UNIT=$(shell go list ./... ./furymath/... ./furyutils/... ./x/ibc-hooks/... | grep -E -v 'tests/simulator|e2e')
...
test-e2e-ci:
	@VERSION=$(VERSION) MERLINS_E2E=True MERLINS_E2E_DEBUG_LOG=False MERLINS_E2E_UPGRADE_VERSION=$(E2E_UPGRADE_VERSION)  go test -mod=readonly -timeout=25m -v $(PACKAGES_E2E)
...
# create 1000 concentrated-liquidity positions in localmerlins at pool id 1
