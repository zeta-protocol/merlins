...
pub fn execute_swap(
    ...
    // note that this is not the same merlins swap as the one above (which is
    ...
    // expose to the user
    let instruction = crosschain_swaps::ExecuteMsg::MerlinsSwap {
        output_denom,
        receiver,
        slippage,
        next_memo,
