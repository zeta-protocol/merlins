...
cfuryvisor/genesis folder and v13 folder.

```{.sh}
cp $GOPATH/bin/merlinsd ~/.merlinsd/cfuryvisor/genesis/bin
mkdir -p ~/.merlinsd/cfuryvisor/upgrades/v13/bin
cp $GOPATH/bin/merlinsd ~/.merlinsd/cfuryvisor/upgrades/v13/bin
```

Cfuryvisor is now ready to be set up for v14.
...
make build
cp build/merlinsd ~/.merlinsd/cfuryvisor/upgrades/v14/bin
```

Now, at the upgrade height, Cfuryvisor will upgrade to the v14 binary
