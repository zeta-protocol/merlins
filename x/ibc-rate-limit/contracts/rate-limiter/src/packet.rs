...
// merlins-std doesn't currently support the SupplyOf query, so I'm defining it localy so it can be used to obtain the channel value
#[derive(
    Clone,
    PartialEq,
    Eq,
...
pub struct QuerySupplyOfResponse {
    ...
    pub amount: ::core::option::Option<merlins_std::types::cosmos::base::v1beta1::Coin>,
}
...
pub mod tests {
    ...
    const WRAPPED_ATOM_ON_MERLINS_TRACE: &str = "transfer/channel-0/uatom";
    const WRAPPED_ATOM_ON_MERLINS_HASH: &str =
    ...
    fn sanity_check() {
        ...
        // uatom sent to merlins
        let packet = Packet::mock(
            format!("channel-141"), // from: hub
            format!("channel-0"),   // to: merlins
            format!("uatom"),
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::In),
            WRAPPED_ATOM_ON_MERLINS_HASH.clone()
        );

        // uatom on merlins sent back to the hub
        let packet = Packet::mock(
            format!("channel-0"),                      // from: merlins
            format!("channel-141"),                    // to: hub
            WRAPPED_ATOM_ON_MERLINS_TRACE.to_string(), // unwrapped before reaching the contract
            0_u128.into(),
        );
        ...
        // fury on the hub sent back to merlins
        // send
        let packet = Packet::mock(
            format!("channel-141"),                // from: hub
            format!("channel-0"),                  // to: merlins
        ...
        // ufury on the hub sent back to the merlins
        let packet = Packet::mock(
            format!("channel-141"),                // from: hub
            format!("channel-0"),                  // to: merlins
            WRAPPED_FURY_ON_HUB_TRACE.to_string(), // unwrapped before reaching the contract
        ...
        // utaom on the merlins sent back to the hub
        // send
        let packet = Packet::mock(
            format!("channel-0"),                      // from: merlins
            format!("channel-141"),                    // to: hub
    ...
    fn sanity_double() {
        ...
        let juno_wrapped_merlins_wrapped_atom_hash =
            "ibc/6CDD4663F2F09CD62285E2D45891FC149A3568E316CE3EBBE201A71A78A69388";

        // Send uatom on stored on merlins to juno
        // send
        let packet = Packet::mock(
            format!("channel-42"),                     // from: merlins
        ...
        let packet = Packet::mock(
            format!("channel-42"), // from: merlins
            format!("channel-0"),  // to: juno
            WRAPPED_ATOM_ON_MERLINS_TRACE.to_string(),
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::In),
            juno_wrapped_merlins_wrapped_atom_hash
        );

        // Send back that multi-wrapped token to merlins
        // send
        let packet = Packet::mock(
            format!("channel-0"),  // from: juno
            format!("channel-42"), // to: merlins
            format!("{}{}", "transfer/channel-0/", WRAPPED_ATOM_ON_MERLINS_TRACE), // unwrapped before reaching the contract
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::Out),
