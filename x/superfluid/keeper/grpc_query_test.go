...
func (suite *KeeperTestSuite) TestTotalDelegationByValidatorForAsset() {
	...
	for _, denom := range denoms {
		...
		for _, result := range res.Assets {
			...
			needed_response_fury, err := suite.App.SuperfluidKeeper.GetSuperfluidFURYTokens(ctx, denom, sdk.NewInt(delegation_amount))
			suite.Require().NoError(err)

			suite.Require().Equal(actual_response_fury, needed_response_fury)
...
func (suite *KeeperTestSuite) TestGRPCQuerySuperfluidDelegations() {
	...
	for _, delegator := range delegatorAddresses {
		...
		multiplier0 := suite.querier.Keeper.GetFuryEquivalentMultiplier(suite.Ctx, denoms[0])
		multiplier1 := suite.querier.Keeper.GetFuryEquivalentMultiplier(suite.Ctx, denoms[1])
		...
		suite.Require().True(res.SuperfluidDelegationRecords[0].EquivalentStakedAmount.IsEqual(sdk.NewCoin("ufury", expectAmount0.RoundInt())))
		suite.Require().True(res.SuperfluidDelegationRecords[1].EquivalentStakedAmount.IsEqual(sdk.NewCoin("ufury", expectAmount1.RoundInt())))
	}
...
func (suite *KeeperTestSuite) TestGRPCQueryTotalDelegationByDelegator() {
	...
	for _, delegator := range delegatorAddresses {
		...
		suite.Require().True(res.TotalEquivalentStakedAmount.IsEqual(total_fury_equivalent))
	}
}
