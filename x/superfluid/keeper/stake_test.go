...
func (suite *KeeperTestSuite) TestValidateLockForSFDelegate() {
	...
	tests := []struct {
		...
		},
		{
			...
			superfluidAssetToSet: types.SuperfluidAsset{Denom: DefaultGammAsset, AssetType: types.SuperfluidAssetTypeLPShare},
			expectedErr:          sdkerrors.Wrapf(types.ErrNonSuperfluidAsset, "denom: %s", "ufury"),
		},
		{
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			for _, lock := range locks {
				...
				furyAmount, err := suite.App.SuperfluidKeeper.GetSuperfluidFURYTokens(suite.Ctx, intermediaryAcc.Denom, tc.unlockAmount)
				suite.Require().NoError(err)
				...
				suite.Require().True(furyAmount.IsPositive())
				supplyAfter := suite.App.BankKeeper.GetSupply(suite.Ctx, bondDenom)
				suite.Require().Equal(supplyAfter, supplyBefore.Sub(sdk.NewCoin(bondDenom, furyAmount)))
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			for denom, multiplier := range tc.multipliersByDenom {
				suite.App.SuperfluidKeeper.SetFuryEquivalentMultiplier(suite.Ctx, 2, denom, multiplier)
			}
			...
			for interAccIndex, intermediaryAcc := range intermediaryAccs {
				...
				expAmount := suite.App.SuperfluidKeeper.GetRiskAdjustedFuryValue(suite.Ctx, decAmt.RoundInt())
