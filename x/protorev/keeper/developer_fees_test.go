	...
	cases := []struct {
		...
		},
		{
			...
			alterState: func() {
				...
				err := suite.pseudoExecuteTrade(types.MerlinsDenomination, sdk.NewInt(2000), 0)
				suite.Require().NoError(err)
			},
			expectedErr:   false,
			expectedCoins: sdk.NewCoins(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(400))),
		},
		{
			...
			expectedErr:   false,
			expectedCoins: sdk.NewCoins(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(400)), sdk.NewCoin("Atom", sdk.NewInt(400))),
		},
		{
			...
			alterState: func() {
				...
				err = suite.pseudoExecuteTrade(types.MerlinsDenomination, sdk.NewInt(2000), 366)
				suite.Require().NoError(err)

				// Trade 4 after year 2
				err = suite.pseudoExecuteTrade(types.MerlinsDenomination, sdk.NewInt(2000), 366*2)
				suite.Require().NoError(err)
			},
			expectedErr:   false,
			expectedCoins: sdk.NewCoins(sdk.NewCoin("Atom", sdk.NewInt(400)), sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(700))),
		},
	}
...
func (suite *KeeperTestSuite) TestUpdateDeveloperFees() {
	cases := []struct {
	...
	}{
		{
			...
			alterState:  func() {},
			expected:    sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(40)),
		},
		{
			...
			expected: sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(20)),
		},
		{
			...
			expected: sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(10)),
		},
		{
