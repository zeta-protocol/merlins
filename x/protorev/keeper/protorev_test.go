...
func (suite *KeeperTestSuite) TestGetTokenPairArbRoutes() {
	...
	_, err := suite.App.ProtoRevKeeper.GetTokenPairArbRoutes(suite.Ctx, "fury", "abc")
...
func (suite *KeeperTestSuite) TestGetAllBaseDenoms() {
	...
	suite.Require().Equal(baseDenoms[0].Denom, types.MerlinsDenomination)
	...
	suite.App.ProtoRevKeeper.SetBaseDenoms(suite.Ctx, []types.BaseDenom{{Denom: "fury"}, {Denom: "atom"}, {Denom: "weth"}})
	baseDenoms, err = suite.App.ProtoRevKeeper.GetAllBaseDenoms(suite.Ctx)
	suite.Require().NoError(err)
	suite.Require().Equal(3, len(baseDenoms))
	suite.Require().Equal(baseDenoms[0].Denom, "fury")
	suite.Require().Equal(baseDenoms[1].Denom, "atom")
...
func (suite *KeeperTestSuite) TestGetPoolForDenomPair() {
	...
	suite.App.ProtoRevKeeper.SetPoolForDenomPair(suite.Ctx, "Atom", types.MerlinsDenomination, 1000)
	pool, err := suite.App.ProtoRevKeeper.GetPoolForDenomPair(suite.Ctx, "Atom", types.MerlinsDenomination)
	suite.Require().NoError(err)
	...
	suite.App.ProtoRevKeeper.SetPoolForDenomPair(suite.Ctx, types.MerlinsDenomination, "Atom", 3000)
	pool, err = suite.App.ProtoRevKeeper.GetPoolForDenomPair(suite.Ctx, types.MerlinsDenomination, "Atom")
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestGetDeveloperFees() {
	...
	furyFees, err := suite.App.ProtoRevKeeper.GetDeveloperFees(suite.Ctx, types.MerlinsDenomination)
	suite.Require().Error(err)
	suite.Require().Equal(sdk.Coin{}, furyFees)
	...
	err = suite.App.ProtoRevKeeper.SetDeveloperFees(suite.Ctx, sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(100)))
	suite.Require().NoError(err)
	...
	furyFees, err = suite.App.ProtoRevKeeper.GetDeveloperFees(suite.Ctx, types.MerlinsDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(100)), furyFees)
	atomFees, err = suite.App.ProtoRevKeeper.GetDeveloperFees(suite.Ctx, "Atom")
	suite.Require().NoError(err)
	...
	suite.Require().Contains(fees, furyFees)
...
func (suite *KeeperTestSuite) TestDeleteDeveloperFees() {
	...
	suite.App.ProtoRevKeeper.DeleteDeveloperFees(suite.Ctx, types.MerlinsDenomination)
