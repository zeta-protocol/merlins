...
func (suite *KeeperTestSuite) TestGetProfitsByDenom() {
	// Should be zero by default
	profits, err := suite.App.ProtoRevKeeper.GetProfitsByDenom(suite.Ctx, types.MerlinsDenomination)
	suite.Require().Error(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.ZeroInt()), profits)

	// Pseudo execute a trade
	err = suite.App.ProtoRevKeeper.UpdateProfitsByDenom(suite.Ctx, types.MerlinsDenomination, sdk.NewInt(9000))
	suite.Require().NoError(err)

	// Check the updated result
	profits, err = suite.App.ProtoRevKeeper.GetProfitsByDenom(suite.Ctx, types.MerlinsDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(9000)), profits)

	// Pseudo execute a second trade
	err = suite.App.ProtoRevKeeper.UpdateProfitsByDenom(suite.Ctx, types.MerlinsDenomination, sdk.NewInt(5000))
	suite.Require().NoError(err)

	// Check the updated result after the second trade
	profits, err = suite.App.ProtoRevKeeper.GetProfitsByDenom(suite.Ctx, types.MerlinsDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(14000)), profits)

	// Check the result of GetAllProfits
	allProfits := suite.App.ProtoRevKeeper.GetAllProfits(suite.Ctx)
	suite.Require().Equal([]sdk.Coin{{Denom: types.MerlinsDenomination, Amount: sdk.NewInt(14000)}}, allProfits)
	...
	// Check the result of GetAllProfits
	allProfits = suite.App.ProtoRevKeeper.GetAllProfits(suite.Ctx)
	suite.Require().Equal([]sdk.Coin{{Denom: "Atom", Amount: sdk.NewInt(1000)}, {Denom: types.MerlinsDenomination, Amount: sdk.NewInt(14000)}}, allProfits)
}
...
func (suite *KeeperTestSuite) TestGetProfitsByRoute() {
	...
	profit, err := suite.App.ProtoRevKeeper.GetProfitsByRoute(suite.Ctx, []uint64{1, 2, 3}, types.MerlinsDenomination)
	suite.Require().Error(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.ZeroInt()), profit)

	// Pseudo execute a trade
	err = suite.App.ProtoRevKeeper.UpdateProfitsByRoute(suite.Ctx, []uint64{1, 2, 3}, types.MerlinsDenomination, sdk.NewInt(1000))
	suite.Require().NoError(err)

	// Check the updated result
	profit, err = suite.App.ProtoRevKeeper.GetProfitsByRoute(suite.Ctx, []uint64{1, 2, 3}, types.MerlinsDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(1000)), profit)

	// Check the result of GetAllProfitsByRoute
	profits = suite.App.ProtoRevKeeper.GetAllProfitsByRoute(suite.Ctx, []uint64{1, 2, 3})
	suite.Require().Equal([]sdk.Coin{{Denom: types.MerlinsDenomination, Amount: sdk.NewInt(1000)}}, profits)
	...
	// Check the result of GetAllProfitsByRoute
	profits = suite.App.ProtoRevKeeper.GetAllProfitsByRoute(suite.Ctx, []uint64{1, 2, 3})
	suite.Require().Contains(profits, sdk.Coin{Denom: types.MerlinsDenomination, Amount: sdk.NewInt(1000)})
...
func (suite *KeeperTestSuite) TestUpdateStatistics() {
	...
	// Check the result of GetProfitsByRoute
	profit, err = suite.App.ProtoRevKeeper.GetProfitsByRoute(suite.Ctx, []uint64{2, 3, 4}, types.MerlinsDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinsDenomination, sdk.NewInt(1100)), profit)
