...
func approxDecEqual(a, b, tol furymath.BigDec) bool {
	return (a.Sub(b).Abs()).LTE(tol)
}

var (
	twodec      = furymath.MustNewDecFromStr("2.0")
	k_threshold = furymath.NewDecWithPrec(1, 1) // Correct within a factor of 1 * 10^{-1}
)
...
func oneMinus(swapFee sdk.Dec) furymath.BigDec {
	return furymath.BigDecFromSDKDec(sdk.OneDec().Sub(swapFee))
}
