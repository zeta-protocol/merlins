...
```sh
merlinsd tx lockup lock-tokens [tokens] --duration --from --chain-id
```
...
```bash
merlinsd tx lockup lock-tokens 15527546134174465309gamm/pool/3 --duration="24h" --from WALLET_NAME --chain-id merlins-1
```

To lockup `25.527546134174465309gamm/pool/13` tokens for a `one week` bonding period from `WALLET_NAME` on the merlins testnet:

```bash
merlinsd tx lockup lock-tokens 25527546134174465309gamm/pool/13 --duration="168h" --from WALLET_NAME --chain-id fury-test-4
```
...
```bash
merlinsd tx lockup lock-tokens 35527546134174465309gamm/pool/197 --duration="336h" --from WALLET_NAME --chain-id merlins-1
```
:::
...
```sh
merlinsd tx lockup begin-unlock-by-id [id] --from --chain-id
```
...
```bash
merlinsd tx lockup begin-unlock-by-id 75 --from WALLET_NAME --chain-id merlins-1
```
:::
::: warning Note
...
```sh
merlinsd tx lockup begin-unlock-tokens --from --chain-id
```
...
```bash
merlinsd tx lockup begin-unlock-tokens --from=WALLET_NAME --chain-id=merlins-1 --yes
```
:::
...
```bash
merlinsd query lockup account-locked-beforetime ADDRESS 1639971082
```
...
```bash
merlinsd query lockup account-locked-beforetime ADDRESS 1641094282
```
...
```sh
merlinsd query lockup account-locked-coins [address]
```

:::: details Example

```bash
merlinsd query lockup account-locked-coins fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259
```
...
```sh
merlinsd query lockup account-locked-longer-duration [address] [duration]
```
...
```bash
merlinsd query lockup account-locked-longer-duration fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 24h
```
...
```sh
merlinsd query lockup account-locked-longer-duration-denom [address] [duration] [denom]
```
...
```bash
merlinsd query lockup account-locked-longer-duration-denom fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 24h gamm/pool/3
```
...
```sh
merlinsd query lockup account-locked-longer-duration-not-unlocking [address] [duration]
```
...
```bash
merlinsd query lockup account-locked-longer-duration-not-unlocking fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 24h
```
...
```bash
merlinsd query lockup account-locked-pasttime [address] [timestamp]
```
...
```bash
merlinsd query lockup account-locked-pasttime fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 1639971082
```
...
```bash
merlinsd query lockup account-locked-pasttime-denom fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 [timestamp] [denom]
```
...
```bash
merlinsd query lockup account-locked-pasttime-denom fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259 1639971082 gamm/pool/3
```
...
```bash
merlinsd query lockup account-unlockable-coins ADDRESS
```
...
```sh
merlinsd query lockup account-unlocking-coins [address]
```

::: details Example

```bash
merlinsd query lockup account-unlocking-coins fury1xqhlshlhs5g0acqgrkafdemvf5kz4pp4c2x259
```
...
```sh
merlinsd query lockup lock-by-id [id]
```
...
```bash
merlinsd query lockup lock-by-id 9
```
...
```sh
merlinsd query lockup module-balance
```
...
```sh
merlinsd query lockup output-all-locks [max lock ID]
```
...
```bash
merlinsd query lockup output-all-locks 1000
```
::: warning Note
...
```sh
merlinsd query lockup total-locked-of-denom [denom] --min-duration
```
...
```bash
merlinsd query lockup total-locked-of-denom gamm/pool/2 --min-duration "336h"
```
...
merlinsd tx lockup lock-tokens 200stake --duration="86400s" --from=validator --chain-id=testing --keyring-backend=test --yes
...
merlinsd tx lockup begin-unlock-tokens --from=validator --gas=500000 --chain-id=testing --keyring-backend=test --yes

# unlock tokens, NOTE: add more gas when unlocking more than two locks in a same command
merlinsd tx lockup unlock-tokens --from=validator --gas=500000 --chain-id=testing --keyring-backend=test --yes

# unlock specific period lock
merlinsd tx lockup unlock-by-id 1 --from=validator --chain-id=testing --keyring-backend=test --yes

# account balance
merlinsd query bank balances $(merlinsd keys show -a validator --keyring-backend=test)
...
# query lock by id
merlinsd query lockup lock-by-id 1

# query account unlockable coins
merlinsd query lockup account-unlockable-coins $(merlinsd keys show -a validator --keyring-backend=test)

# query account locks by denom past time
merlinsd query lockup account-locked-pasttime-denom $(merlinsd keys show -a validator --keyring-backend=test) 1611879610 stake

# query account locks past time
merlinsd query lockup account-locked-pasttime $(merlinsd keys show -a validator --keyring-backend=test) 1611879610

# query account locks by denom with longer duration
merlinsd query lockup account-locked-longer-duration-denom $(merlinsd keys show -a validator --keyring-backend=test) 5.1s stake

# query account locks with longer duration
merlinsd query lockup account-locked-longer-duration $(merlinsd keys show -a validator --keyring-backend=test) 5.1s

# query account locked coins
merlinsd query lockup account-locked-coins $(merlinsd keys show -a validator --keyring-backend=test)

# query account locks before time
merlinsd query lockup account-locked-beforetime $(merlinsd keys show -a validator --keyring-backend=test) 1611879610
```
