...
func TickToPrice(tickIndex sdk.Int) (price sdk.Dec, err error) {
	...
	price = PowTenInternal(geometricExponentDelta).Add(furymath.BigDecFromSDKDec(numAdditiveTicks).Mul(currentAdditiveIncrementInTicks).SDKDec())
...
func powTenBigDec(exponent sdk.Int) furymath.BigDec {
	if exponent.GTE(sdk.ZeroInt()) {
		return furymath.NewBigDec(10).PowerInteger(exponent.Uint64())
	}
	return furymath.OneDec().Quo(furymath.NewBigDec(10).PowerInteger(exponent.Abs().Uint64()))
}
...
func CalculatePriceToTick(price sdk.Dec) (tickIndex sdk.Int) {
	...
	if price.GT(sdk.OneDec()) {
		for currentPrice.LT(price) {
			currentAdditiveIncrementInTicks = powTenBigDec(exponentAtCurrentTick)
			maxPriceForCurrentAdditiveIncrementInTicks := furymath.BigDecFromSDKDec(geometricExponentIncrementDistanceInTicks).Mul(currentAdditiveIncrementInTicks)
	...
	ticksToBeFulfilledByExponentAtCurrentTick := furymath.BigDecFromSDKDec(price.Sub(currentPrice)).Quo(currentAdditiveIncrementInTicks)
