...
var (
	...
	additiveFeeGrowthGlobalErrTolerance = furymath.ErrTolerance{
		// 2 * 10^-18
			...
			if test.expectErr {
				...
				s.Require().Equal(0,
					additiveFeeGrowthGlobalErrTolerance.CompareBigDec(
						furymath.BigDecFromSDKDec(test.expectedFeeGrowthAccumulatorValue),
						furymath.BigDecFromSDKDec(feeAccum.GetValue().AmountOf(test.tokenIn.Denom)),
					),
		...
		s.Run(name, func() {
			...
			if test.expectErr {
				...
				s.Require().Equal(0,
					additiveFeeGrowthGlobalErrTolerance.CompareBigDec(
						furymath.BigDecFromSDKDec(test.expectedFeeGrowthAccumulatorValue),
						furymath.BigDecFromSDKDec(actualValue),
					),
	...
	for name, test := range tests {
		...
		s.Run(name, func() {
			...
			s.Require().Equal(1,
				additiveFeeGrowthGlobalErrTolerance.CompareBigDec(
					furymath.BigDecFromSDKDec(test.expectedFeeGrowthAccumulatorValue),
					furymath.BigDecFromSDKDec(feeAccum.GetValue().AmountOf(test.tokenInDenom)),
				),
			)
...
func (s *KeeperTestSuite) TestFunctionalSwaps() {
	...
	actualSqrtPrice := furymath.BigDecFromSDKDec(clPool.GetCurrentSqrtPrice())
