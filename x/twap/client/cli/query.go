...
import (
	...
	poolmanager "github.com/merlins-labs/merlins/v15/x/poolmanager/client/queryproto"
	"github.com/merlins-labs/merlins/v15/x/twap/client/queryproto"
	"github.com/merlins-labs/merlins/v15/x/twap/types"
)
...
func GetQueryArithmeticCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Aliases: []string{"twap"},
		Long: furycli.FormatLongDescDirect(`Query arithmetic twap for pool. Start time must be unix time. End time can be unix time or duration.
...
func GetQueryGeometricCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: furycli.FormatLongDescDirect(`Query geometric twap for pool. Start time must be unix time. End time can be unix time or duration.
...
func twapQueryParseArgs(args []string) (poolId uint64, baseDenom string, startTime time.Time, endTime time.Time, err error) {
	// boilerplate parse fields
	// <UINT PARSE>
	poolId, err = furycli.ParseUint(args[0], "poolId")
	if err != nil {
		return
	}
	...
	// <UNIX TIME PARSE>
	startTime, err = furycli.ParseUnixTime(args[2], "start time")
	if err != nil {
		return
	}
	...
	endTime, err = furycli.ParseUnixTime(args[3], "end time")
	if err != nil {
