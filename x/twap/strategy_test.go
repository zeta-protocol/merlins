	...
	for name, test := range tests {
		s.Run(name, func() {
			for _, twapStrategy := range test.twapStrategies {
				...
				furyassert.DecApproxEq(s.T(), test.expTwap, actualTwap, furymath.GetPowPrecision())
			}
		})
	}
}
...
func (s *TestSuite) TestComputeArithmeticStrategyTwap() {
	...
	for name, test := range tests {
		s.Run(name, func() {
			furyassert.ConditionalPanic(s.T(), test.expPanic, func() {
...
func (s *TestSuite) TestComputeGeometricStrategyTwap() {
	...
	for name, tc := range tests {
		tc := tc
		s.Run(name, func() {
			furyassert.ConditionalPanic(s.T(), tc.expPanic, func() {
	...
	for name, test := range tests {
		s.Run(name, func() {
			for i, startRec := range test.startRecord {
				...
				furyassert.DecApproxEq(s.T(), test.expTwap[i], actualTwap, errTolerance)
			}
		})
	}
}
