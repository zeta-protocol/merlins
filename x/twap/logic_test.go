...
import (
	...
	"github.com/merlins-labs/merlins/v15/x/twap/types/twapmock"
)
...
func TestRecordWithUpdatedAccumulators(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			furyassert.ConditionalPanic(t, test.expectPanic, func() {
	...
	tests := map[string]struct {
		...
		"two-asset; pre-set at t and t + 1, new record inserted between existing": {
			...
			expectedHistoricalRecords: []expectedResults{
				...
				// https://github.com/merlins-labs/merlins/issues/2686
				{
					spotPriceA:   sdk.OneDec(),
					spotPriceB:   sdk.OneDec().Add(sdk.OneDec()),
					isMostRecent: true,
...
func (s *TestSuite) TestComputeArithmeticTwapWithSpotPriceError() {
	...
	for name, test := range tests {
		s.Run(name, func() {
			...
			furyassert.ConditionalError(s.T(), test.expErr, err)
		})
	}
}
...
func (s *TestSuite) TestTwapLog() {
	...
	for _, tc := range testcases {
		s.Run(tc.name, func() {
			furyassert.ConditionalPanic(s.T(), tc.expectPanic, func() {
