...
import (
	...
	"github.com/merlins-labs/merlins/v15/tests/mocks"
	...
	gamm "github.com/merlins-labs/merlins/v15/x/gamm/keeper"
...
func (suite *KeeperTestSuite) calcOutAmountAsSeparateSwaps(furyFeeReduced bool, routes []poolmanagertypes.SwapAmountOutRoute, tokenOut sdk.Coin) sdk.Coin {
	...
	if furyFeeReduced {
		...
		routeSwapFee, sumOfSwapFees, err := suite.App.PoolManagerKeeper.GetFuryRoutedMultihopTotalSwapFee(suite.Ctx, route)
		suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) calcInAmountAsSeparateSwaps(furyFeeReduced bool, routes []poolmanagertypes.SwapAmountInRoute, tokenIn sdk.Coin) sdk.Coin {
	cacheCtx, _ := suite.Ctx.CacheContext()
	if furyFeeReduced {
		// extract route from swap
