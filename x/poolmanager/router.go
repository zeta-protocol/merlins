...
func (k Keeper) RouteExactAmountIn(
	...
	if k.isFuryRoutedMultihop(ctx, route, routes[0].TokenOutDenom, tokenIn.Denom) {
		isMultiHopRouted = true
		routeSwapFee, sumOfSwapFees, err = k.getFuryRoutedMultihopTotalSwapFee(ctx, route)
		if err != nil {
			return sdk.Int{}, err
		}
	}
...
func (k Keeper) RouteExactAmountOut(ctx sdk.Context,
	...
	// total_swap_fee = total_swap_fee = max(swapfee1, swapfee2)
	// fee_per_pool = total_swap_fee * ((pool_fee) / (swapfee1 + swapfee2))
	if k.isFuryRoutedMultihop(ctx, route, routes[0].TokenInDenom, tokenOut.Denom) {
		isMultiHopRouted = true
		routeSwapFee, sumOfSwapFees, err = k.getFuryRoutedMultihopTotalSwapFee(ctx, route)
		if err != nil {
			return sdk.Int{}, err
	...
	if isMultiHopRouted {
		insExpected, err = k.createFuryMultihopExpectedSwapOuts(ctx, routes, tokenOut, routeSwapFee, sumOfSwapFees)
	} else {
...
func (k Keeper) getFuryRoutedMultihopTotalSwapFee(ctx sdk.Context, route types.MultihopRoute) (
