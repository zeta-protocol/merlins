...
```shell
merlinsd tx gov submit-proposal update-pool-incentives [gaugeIds] [weights]
```
...
```shell
merlinsd tx gov submit-proposal update-pool-incentives 2,3 100,200
```
...
```sh
merlinsd tx poolincentives replace-pool-incentives [gaugeIds] [weights] [flags]
```
...
```bash
merlinsd tx poolincentives replace-pool-incentives proposal.json --from --chain-id
```
...
```sh
merlinsd tx poolincentives update-pool-incentives [gaugeIds] [weights] [flags] --from --chain-id
```
...
```bash
merlinsd tx gov submit-proposal update-pool-incentives proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
```sh
merlinsd query poolincentives distr-info
```
...
```sh
merlinsd query pool-incentives external-incentivized-gauges
```
...
```sh
merlinsd query poolincentives gauge-ids [pool-id] [flags]
```
...
```bash
merlinsd query poolincentives gauge-ids 1
```
...
```sh
merlinsd query poolincentives incentivized-pools [flags]
```

::: details Example

```bash
merlinsd query poolincentives incentivized-pools
```
...
```sh
merlinsd query poolincentives lockable-durations [flags]
```

::: details Example

```bash
merlinsd query poolincentives lockable-durations
```
...
```sh
merlinsd query poolincentives params [flags]
```

::: details Example

```bash
merlinsd query poolincentives params
```
