...
import (
	...
	furyapp "github.com/merlins-labs/merlins/v15/app"
	"github.com/merlins-labs/merlins/v15/x/mint/keeper"
	"github.com/merlins-labs/merlins/v15/x/mint/types"
...
func (suite *KeeperTestSuite) TestAfterEpochEnd() {
	...
	for name, tc := range testcases {
		suite.Run(name, func() {
			...
			furyassert.DecApproxEq(suite.T(), developerAccountBalanceBeforeHook.Amount.Sub(expectedDevRewards.TruncateInt()).ToDec(), developerAccountBalanceAfterHook.Amount.ToDec(), maxArithmeticTolerance)

			// Validate supply.
			furyassert.DecApproxEq(suite.T(), expectedSupply.Add(tc.expectedDistribution).Sub(expectedDevRewards), app.BankKeeper.GetSupply(ctx, sdk.DefaultBondDenom).Amount.ToDec(), maxArithmeticTolerance)

			// Validate supply with offset.
			furyassert.DecApproxEq(suite.T(), expectedSupplyWithOffset.Add(tc.expectedDistribution), app.BankKeeper.GetSupplyWithOffset(ctx, sdk.DefaultBondDenom).Amount.ToDec(), maxArithmeticTolerance)
			...
			if !tc.expectedDistribution.IsZero() {
				...
				furyassert.DecApproxEq(suite.T(), tc.expectedDistribution, mintKeeper.GetMinter(ctx).EpochProvisions, sdk.NewDecWithPrec(1, 6))
			}
		})
	}
}
