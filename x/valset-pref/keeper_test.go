...
func (suite *KeeperTestSuite) SetupLocks(delegator sdk.AccAddress) []lockuptypes.PeriodLock {
	...
	suite.Require().NoError(err)
	workingLock, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, furyToLock, twoWeekDuration)
	suite.Require().NoError(err)
	...
	suite.FundAcc(sdk.AccAddress([]byte("addr5---------------")), furyToLock)
	lockWithDifferentOwner, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, sdk.AccAddress([]byte("addr5---------------")), furyToLock, twoWeekDuration)
	suite.Require().NoError(err)
	...
	suite.Require().NoError(err)
	maxDurationLock, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, furyToLock, morethanTwoWeekDuration)
	suite.Require().NoError(err)

	locks = append(locks, maxDurationLock)

	// unbonding locks
	unbondingLocks, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, furyToLock, twoWeekDuration)
	suite.Require().NoError(err)
	...
	// synthetic locks
	syntheticLocks, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, furyToLock, twoWeekDuration)
	suite.Require().NoError(err)

	err = suite.App.LockupKeeper.CreateSyntheticLockup(suite.Ctx, syntheticLocks.ID, "ufury", time.Minute, true)
	suite.Require().NoError(err)
